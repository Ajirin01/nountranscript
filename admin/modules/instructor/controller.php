...
function savegrade(){
	...
	if ($_POST['total']>=75 AND $_POST['total']<=100){
		$remarks = "Passed";
	}else{
		$remarks= "Failed";
	}
...
for ($i=0; $i<$subjId; $i++){
	...
	foreach ($cur as  $result) {
		...
		// echo json_encode($class->create()) ."<br><br>";
		
		if($class->create()){
			message("Courses are successfully assigned to Lecturer!","info");
			redirect('index.php?view=instSubj&instructorId='.$instructorId.'');
		}
		else{
			message("Could not assign course(s) to lecturer!","error");
			redirect('index.php?view=instSubj&instructorId='.$instructorId.'');
		}
	}
...
function doEdit(){
	...
	if ($_POST['name'] == "" OR $_POST['address'] == "" OR $_POST['email'] == "") {
		...
		$status 		= $_POST['status'];	
			...
			// echo json_encode($inst->update($instid));
			// exit;
			$inst->update($instid);
