...
if ($LNAME == "" && $FNAME == "" && $MNAME == "" && $EMAIL == "" && $natid == "") {
	message('last name, first name, middle name, email and national ID number are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($LNAME == "" && $FNAME == "" && $MNAME == "" && $EMAIL == "") {
	message('last name, first name, middle name and email  are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($LNAME == "" && $FNAME == "" && $MNAME == "") {
	message('last name, first name and middlename are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($LNAME == "" && $FNAME == "") {
	message('Last Name and first name is required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($FNAME == "" && $MNAME == "" && $EMAIL == "" && $natid == "") {
	message('firstname, middlename, email and national ID number  are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($LNAME == "" && $EMAIL == "" && $MNAME == "" && $natid == "") {
	message('lastname, middlename, email and national ID number  are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($MNAME == "" && $EMAIL == "" && $natid == "") {
	message('middlename, email and national  are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($EMAIL == "" && $natid == "") {
	message('email and natoional ID are required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
}elseif ($LNAME == "") {
	...
	message('national ID number is required!', "error");
	redirect ('eportal.php?eportal-page=admission_application');
	...
	if($student->create() && $studdetails->create() && $requirements->create()){
	message('New student created successfully!', "success");
	redirect('eportal.php?eportal-page=profile');	
	}else{
	message('Your record could not be addedd successfully!  Please make sure you fill up all the fields in the primary detail section', "error");
	redirect('eportal.php?eportal-page=admission_application');	
	}
 ...
 @$id=$_POST['selector'];
	...
	function doAssignsubj(){
		...
		for ($i=0;$i<$subjId; $i++){
			...
			message("Student's subjects already Added!","info");//&studentId=20004180&SY=2032-2033&cid=5&sy=31
			redirect('index.php?page=4&studentId='.$studentId.'&SY='.$SY.'&cid='.$cid.'&sy='.$sy.'');
