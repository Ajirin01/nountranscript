...
function doEnroll(){
	...
	if (isset($_POST['savestep1'])){
			 ...
			 if ($istrue == 1){
				...
				message("Students could not be successfully enrolled! Assign now the student's subjects.","error");
				// check_message();
				redirect('index.php?view=enroll&studentId='.$idno.'');
			 }
	...
	*/
	...
	if ($IDNO == "") {
		...
		// if($student->update($_GET['id']) && $studdetails->update($_GET['id']) && $requirements->update($_GET['id'])){
			...
			$message = "Congratulations applicant, you have been given admission
			 please login to your account with these detail ID: '".$IDNO."' and password: '1234' and make sure you 
